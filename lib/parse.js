  ...
  this.pull(26).then(data => {
    ...
    return this.pull(vars.fileNameLength).then(fileName => {
      ...
      this.pull(vars.extraFieldLength).then(extraField => {
        if (vars.compressionMethod === 0) {
          this.pull(vars.compressedSize).then(compressedData => {
            if (hasEntryListener) {
          ...
          if (fileSizeKnown) {
            ...
            if (hasEntryListener) {
              entry.on('finish', this._readRecord.bind(this));
              this.stream(vars.compressedSize).pipe(inflater).pipe(entry);
            } else {
              this.drain(vars.compressedSize, function (err) {
                if (err) {
