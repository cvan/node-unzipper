...
Parse.prototype._readRecord = function () {
  var self = this;
  this._pullStream.pull(4).then(function (data) {
    ...
    if (signature === 0x04034b50) {
      ...
      var err = new Error('invalid signature: 0x' + signature.toString(16));
      self.emit('error', err);
    }
  });
};
...
Parse.prototype._readFile = function () {
  var self = this;
  this._pullStream.pull(26).then(function (data) {
    ...
    return self._pullStream.pull(vars.fileNameLength).then(function(fileName) {
      ...
      self._pullStream.pull(vars.extraFieldLength).then(function (extraField) {
        if (vars.compressionMethod === 0) {
          self._pullStream.pull(vars.compressedSize).then(function (compressedData) {
            if (hasEntryListener) {
          ...
          if (fileSizeKnown) {
            ...
            if (hasEntryListener) {
              ...
              self._pullStream.stream(vars.compressedSize, inflater).pipe(entry);
            } else {
...
Parse.prototype._processDataDescriptor = function (entry) {
  var self = this;
  this._pullStream.pull(16).then(function (data) {
    var vars = binary.parse(data)
...
Parse.prototype._readCentralDirectoryFileHeader = function () {
  var self = this;
  this._pullStream.pull(42).then(function (data) {
    ...
    return self._pullStream.pull(vars.fileNameLength).then(function(fileName) {
      ...
      self._pullStream.pull(vars.extraFieldLength).then(function(extraField) {
        self._pullStream.pull(vars.fileCommentLength).then(function(fileComment) {
          return self._readRecord();
        });
      });
    });
...
Parse.prototype._readEndOfCentralDirectoryRecord = function () {
  var self = this;
  this._pullStream.pull(18).then(function(data) {
    ...
    if (vars.commentLength) {
      setImmediate(function() {
        self._pullStream.pull(vars.commentLength).then(function (comment) {
